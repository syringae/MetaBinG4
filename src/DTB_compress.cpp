...
void usage() {
    cerr << "./DB_compress paths.txt nameFamily.txt database\n";
}

int main(int argc, char **argv) {
    if(argc != 4) { usage(); exit(1); }

    cerr << "[INFO] Starting program..." << endl;

    // --- OPEN INPUT FILE ---
    ifstream fin(argv[1]);
    if (!fin.is_open()) {
        cerr << "[ERROR] Cannot open targets file: " << argv[1] << endl;
        return 1;
    }
    cerr << "[INFO] Opened targets file: " << argv[1] << endl;
    
    
    // --- OPEN FAMILY OUTPUT ---
    ofstream foutF(argv[2]);
    if (!foutF.is_open()) {
        cerr << "[ERROR] Cannot create family output file: " << argv[2] << endl;
        return 1;
    }
    cerr << "[INFO] Created family file: " << argv[2] << endl;

    // --- OPEN DB OUTPUT ---
    std::string outFileName = std::string(argv[3]) + "_prebuild"; 
    std::ofstream foutD(outFileName, std::ofstream::binary); 
    if (!foutD.is_open()) {
        std::cerr << "[ERROR] Cannot create database file: " << outFileName << std::endl; 
        return 1; 
    }
    cerr << "[INFO] Created binary DB file: " << argv[3] << endl;
    ...
    memset(parent, 0, 3000005 * sizeof(int));
    int maxx = 2700000;
    string file;
    int t[8], step = 0, cntFamily = 0;

    // ------------------------------------------------------------
    cerr << "[INFO] Reading targets file: " << argv[1] << endl;
    ...
    while (fin >> file) {
        ...
        if(t[1] != -1 ) {
            int familyID, genusID, speciesID;
        ...
        // if (step % 1000 == 0) {
        //     cerr << "[INFO] Processed " << step << " entries..." << endl;
        // }
    }

    cerr << "[INFO] Finished reading input. Total families: " << cntFamily << endl;
    cerr << "[INFO] RANGE parameter = " << RANGE << endl;
    cerr << "[INFO] Max index used: " << maxx << endl;

    // ------------------------------------------------------------
    for (int i = 1; i <= cntFamily; i++) {
        // cerr << "[INFO] Processing family " << i << "/" << cntFamily 
        //      << " (FamilyTID=" << idFamily[i] << ")" << endl;

        // long long before = foutD.tellp();
        solveFamily(idFamily[i], vectorFamily[i], foutF, foutD);
        // long long after = foutD.tellp();

        // cerr << "[DEBUG] Family " << idFamily[i] 
        //      << " wrote " << (after - before) << " bytes to DB file." << endl;
        if (!foutD.good()) {
            cerr << "[ERROR] Writing to DB file failed at family " << idFamily[i] << endl;
            return 1;
        }

        // cerr << "[INFO] Finished family " << idFamily[i] << endl;
    }
    
    if (RANGE == 5) cerr << "[INFO] Program selects X = 20%" << endl;
    else if (RANGE == 7) cerr << "[INFO] Program selects X = 15%" << endl;
    else if (RANGE == 10) cerr << "[INFO] Program selects X = 10%" << endl;
    else if (RANGE == 20) cerr << "[INFO] Program selects X = 5%" << endl;
    cerr << endl << maxx << endl;
    
    cerr << "[INFO] All families processed." << endl;
    cerr << "[RESULT] Total genome KMERs = " << sumKMERGenome << endl;
    cerr << "[RESULT] Total KMERs inserted into sets = " << sumSet << endl;
    cerr << "[RESULT] Final KMERs written = " << sumFinalKMER << endl;

    cerr << endl << endl;
    foutF.close();
    foutD.close();
    cerr << "[INFO] Output files closed successfully." << endl;
    return 0;
}
