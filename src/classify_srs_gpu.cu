...
void countKmer(char *seq,int kmer,char *frag,float *seqm,int buffi,int seqvlen){
	...
	for (size_t i = 0; i + kmer <= seqlen; i++){
        int legal = 1;
        ...
        if(legal == 1){
            // Copy exactly kmer characters then add the null terminator.
            strncpy(frag, seq + i, kmer);
            frag[kmer] = '\0';
            // Increase counter at the proper index. Note: buffi * seqvlen, not buffi*(seqvlen)
            seqm[calKmerIndex(frag) + buffi * seqvlen] += 1;
            memset(frag, 0, kmer + 1);
        }
    }
}
...
int matMultiplication(int mrow,int buff,float* scorelist,float* d_scorelist,
					int seqvlen,float* seqm,float* d_seqm,float* d_datad, 
					char ** datas,char** titlel,float* sigmaG,int* statsNum,
					int flag,float alpha,float* d_sigmaG,int* d_index,int* index,
					int buffi,FILE *fp){
	cublasStatus status;
...
void *classFun(void *arg){
    ...
    char* seq = (char*)calloc(MAX_SEQ_LENGTH, sizeof(char));
    if (!seq) {
        fprintf(stderr, "Memory allocation failed for seq\n");
        pthread_exit(NULL);
    }
	...
	memset(title, 0, 1000 * sizeof(char));

	char** titlel=(char**)malloc(buff*sizeof(char*));
	for(int i=0;i<buff;i++){
		titlel[i]=(char*)malloc((1000)*sizeof(char));
		memset(titlel[i], 0, 1000* sizeof(char));
	}

	char* frag = (char*)malloc(sizeof(char) * (kmer + 1));
	if (!frag) {
        fprintf(stderr, "Memory allocation failed for frag\n");
        pthread_exit(NULL);
    }
    memset(frag, 0, kmer + 1);
	...
	char** datas_fun=(char**)malloc(mrow*sizeof(char*));
	for(i=0;i<mrow;i++){
		for(j=0;j<mrow;j++){
			datas_fun[i]=(char*)malloc((strlen(datas[i])+1)*sizeof(char));
			strcpy(datas_fun[i],datas[i]);
		}
	}

	float* datad_fun=(float*)malloc((mcolmax-1)*mrow*sizeof(float));
	memcpy(datad_fun,datad,sizeof(float)*mrow*(mcolmax-1));
	float* d_datad=0;
	cublasAlloc(mrow*(mcolmax-1), sizeof(float),(void**)&d_datad);
	cublasSetVector(mrow*(mcolmax-1), sizeof(float), datad_fun, 1, d_datad, 1);
    ...
    while(fgets(line,(10000+(kmer+2)*seqvlen),fp)!=NULL){
            ...
            if(line[0]=='>'){
					if(title[0]=='>'){
						...
						if(buffi==buff){
							buffi=0;
							memset(scorelist, 0, sizeof(float) * buff * mrow);		   
							matMultiplication(mrow,buff,scorelist,d_scorelist,seqvlen,seqm,
											d_seqm,d_datad,datas_fun,titlel,sigmaG_fun,
											statsNumthread,flag,alpha,d_sigmaG,d_index,
											index,buffi,fp1);
							memset(seqm, 0, sizeof(float) * seqvlen * buff);
						}
					}
					memset(title,0,1000);
					strncpy(title,line,strlen(line));
					memset(seq, 0, MAX_SEQ_LENGTH);
					memset(line,0,(10000+(kmer+2)*((unsigned int)pow(4.0,kmer))));			
				}else{
					if(strlen(seq) + strlen(line) < MAX_SEQ_LENGTH - 1) {
						strcat(seq, line);
					} else {
						fprintf(stderr, "Sequence too long, truncating\n");
					}
				}
	...
	for(int i=0;i<mrow;i++){free(datas_fun[i]);}
	free(titlel);
	free(datas_fun);
	free(line);			
	free(title);			
...
int main(int argc,char ** argv){
	if(argc==5){
		...
		//======================================================================================
		datas=(char**)malloc(mrow*sizeof(char*));

		datad=(float*)malloc((mcolmax-1)*mrow*sizeof(float));
		...
		while(fgets(line,(10000+(kmer+2)*((unsigned int)pow(4.0,kmer))),refp1)!=NULL){
			...
			for(p=strtok(line,"\t\n");p;p=strtok(NULL,"\t\n")){
				if(tcol<1){
					datas[trow*1+tcol]=(char*)malloc((strlen(p)+1)*sizeof(char));
					strcpy(datas[trow*1+tcol],p);
				}else{
					datad[trow*(mcolmax-1)+tcol-1]=atof(p);
				}
				tcol++;
				if(tcol==mcolmax){
					trow++;
		...
		free(percentl);
		free(statsNum);
		// for(int i=0;i<mrow;i++){free(datas[i]);}	
		for(int i = 0; i < mrow; i++) {
			free(datas[i]);
		}	
		free(datas);	
		...
		int ret = system(cmd);
		if (ret != 0) {
			fprintf(stderr, "[WARN] command failed (cmd): %s (rc=%d)\n", cmd, ret);
		}
		char cmd1[100];
		sprintf(cmd1,"rm %s_tmp*",argv[4]);
		int ret1 = system(cmd1);
		if (ret1 != 0) {
			fprintf(stderr, "[WARN] command failed (cmd1): %s (rc=%d)\n", cmd1, ret1);
		}
