...
void *classFun(void *arg){
    ...
    char* line = (char*)malloc((10000 + (kmer + 2)) * seqvlen * sizeof(char));
    memset(line, 0, (10000 + (kmer + 2)) * seqvlen * sizeof(char));

    int buffi=0;

    char* seq = (char*)malloc((10000 + (kmer + 2)) * seqvlen * sizeof(char));
    memset(seq, 0, (10000 + (kmer + 2)) * seqvlen * sizeof(char));
    ...
    for (int i = 0; i < buff; i++) {
        ...
        memset(titlel[i], 0, 1000);
    }

	char* frag = (char*)malloc(sizeof(char) * 10);
    memset(frag, 0, sizeof(char) * 10);
    ...
    memset(index, 0, sizeof(int) * buff);
    ...
    while (fgets(line, (10000 + (kmer + 2)) * seqvlen, fp) != NULL) {
        ...
        if (line[0] == '>') {
			...
			size_t remainingSize = 10000+(kmer+2)*((unsigned int)pow(4.0,kmer)); //sizeof(seq) - strlen(seq) - 1; 
			strncpy(seq,strncat(seq, line, remainingSize),strlen(seq));
		}		
    }
