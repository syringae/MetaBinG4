...
int ClassifySequence(string &s, HashTable &HT) {
    ...
    for (size_t i = 1; i < static_cast<size_t>(lenSeq - KMER); ++i){
...
int main (int argc, char **argv) {
    ...
    int valFamily, valGenus, valSpecies;
    ...
    string dbDir(argv[1]);
    string dbPrefix = dbDir + "/lrs_db"; 

    if (!fs::exists(dbPrefix + "_Size") ||
        !fs::exists(dbPrefix + "_Suffix") ||
        !fs::exists(dbPrefix + "_Taxo")) {
        cerr << "[ERROR] Database files not found in: " << dbDir << endl;
        cerr << "Expected: " << dbPrefix << "_Size, _Suffix, _Taxo" << endl;
        exit(1);
    }

    cerr << "Loading database from: " << dbPrefix << "..." << endl;
